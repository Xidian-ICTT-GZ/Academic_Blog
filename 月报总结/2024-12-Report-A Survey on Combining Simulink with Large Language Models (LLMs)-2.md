...
上月的研究工作主要聚焦于大语言模型（LLMs）与Simulink结合的多项核心方向，取得了一系列进展。在模型切片领域，调研了如何基于自然语言需求利用LLMs提取Simulink模型的关键切片，分析了文本化粒度和提示策略（如思维链、零样本学习）对生成准确性的影响。在测试与验证自动化方面，研究了LLMs生成单元测试用例的有效性，并探索了通过强化学习优化测试用例质量的机制，进一步提升测试效率与覆盖率。

但上月并未涉及数据集的构建与智能检索和思维链调研，因此本次调研新增了数据集的构建与智能检索方向并首次探索了思维链在复杂推理任务中的应用潜力，来为Simulink模型语义理解和推理任务中的扩展应用提供理论基础。

### 调研内容

| 序号 | 调研方向                       | 上月调研内容                                                 | 本月调研内容                                                 |
| ---- | ------------------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 1    | LLM + Slicing                  | 调研了基于自然语言需求的Simulink模型切片方法                 | 继续深入的调研Slicing方法的早期工作，同时深化了对模型重用的流程优化和技术应用的探索 |
| 2    | Simulink模型的测试与验证自动化 | 调研了LLMs生成单元测试用例的有效性分析和强化学习优化测试用例质量的机制 | 继续调研Simulink模型的测试与验证自动化内容，包括基于测试序列和评估块的仿真测试工具、故障注入与变异引擎在模拟和分析系统故障中的应用和基于软件需求和领域知识的测试工具 |
| 3    | Simulink数据集的构建与智能检索 | 上月调研并未涉及此方面，但高质量数据集对模型理解和检索有着重要作用，为智能搜索引擎构建做好准备 | 调研了Simulink语料库的构建与智能检索，包括研究开源模型及变化管理和Simulink模型语料库的可复制性研究，并探索了LLM与Mongo数据库结合的实践 |
| 4    | 思维链与推理任务               | 上月对思维链只是简单提及，但思维链在分解复杂问题中很重要，构建清晰的思维链可以提升系统在推理、解释和复杂任务处理中的表现 | 考虑到LLM+Slicing中也使用到思维链，为了提高使用思维链方法识别Simulink模型的准确率，本月调研思维链与推理任务，包括Zero-shot-CoT方法在零样本推理任务中的表现，以及通过程序生成提示提升语言模型在复杂数学问题上的推理性能 |

### 相关工作一：LLM + Slicing 

上个月主要调研了基于自然语言需求的Simulink模型切片方法，特别是如何利用大语言模型（LLM）提取与需求相关的模型切片。调研内容包括对Simulink模型文本化的不同粒度影响，以及提示策略（如思维链、零样本学习）对切片生成准确性的作用。

本月继续深入的调研Slicing方法的早期工作，同时深化了对模型重用的流程优化和技术应用的探索，为后续研究奠定了理论基础。

### 相关论文一

### Slicing MATLAB Simulink Models<sup>[1]</sup>

论文链接：[Slicing MATLAB Simulink Models](https://ieeexplore.ieee.org/abstract/document/6227161)
...
<img src="https://github.com/user-attachments/assets/00c1bc61-e423-4bed-bff0-d260df4a6ce5" alt="image" style="zoom:80%;" />
...
- 模型的根上下文和条件子系统 S 的执行上下文，即CEC_S。
  ...
  <img src="https://github.com/user-attachments/assets/bae1288d-70d1-4a2b-bd5e-aa6b1e4d80cc" alt="image" style="zoom:80%;" />

  对应的控制流程图：

  <img src="https://github.com/user-attachments/assets/e52a46c6-9a87-4257-afd1-ebfa0181b1cd" alt="image" style="zoom:80%;" />
...
<img src="https://github.com/user-attachments/assets/5b20f2be-d3fb-47a1-bf4f-ddf6c7b7bf1f" alt="image" style="zoom:80%;" />
...
<img src="https://github.com/user-attachments/assets/2bdb656b-ffea-4677-ab05-8a00c2178e9c" alt="image" style="zoom:80%;" />
...
<img src="https://github.com/user-attachments/assets/12809c20-29ec-4f5f-abcd-b965aae90d13" alt="image" style="zoom:80%;" />
...
### Towards the reuse of physical models within the development life-cycle: a case study of Simulink models<sup>[2]</sup>
...
<img src="https://github.com/Anorexia16/PicStream/releases/download/asd2/2024-12-26.215157.png" alt="PhysicalComponent2Simulink系统设计与实现" style="zoom:80%;" />
...
2. **结果分析**
   <img src="https://github.com/Anorexia16/PicStream/releases/download/asd2/2024-12-26.225444.png" alt="img" style="zoom:80%;" />
...
### Approaches for Simulation Model Reuse in Systems Design — A Review<sup>[3]</sup>
...
### 相关工作二：Simulink模型的测试与验证自动化

上个月进行了关于大语言模型在单元测试生成中的研究，包括LLMs生成单元测试用例的有效性分析，以及通过强化学习优化测试用例质量的机制探索。

本月继续调研Simulink模型的测试与验证自动化内容，重点研究了基于测试序列和评估块的仿真测试工具，以及故障注入与变异引擎在模拟和分析系统故障中的应用。此外，还分析了基于软件需求和领域知识的测试工具，探索通过适应度函数生成符合需求的测试用例的具体方法，为Simulink模型测试效率与覆盖率的提升做好了准备。

### 相关论文一

### Simulation-Based Testing of Simulink Models With Test Sequence and Test Assessment Blocks<sup>[4]</sup>

论文链接：[Simulation-Based Testing of Simulink Models With Test Sequence and Test Assessment Blocks](https://www.researchgate.net/profile/Tomasz-Tarczewski/publication/386181393_Large_Language_Model-Based_Tuning_Assistant_for_Variable_Speed_PMSM_Drive_with_Cascade_Control_Structure/links/674d71bda7fbc259f1a5c507/Large-Language-Model-Based-Tuning-Assistant-for-Variable-Speed-PMSM-Drive-with-Cascade-Control-Structure.pdf)
...
<img src="https://github.com/Anorexia16/PicStream/releases/download/asd2/1.png" alt="img" style="zoom:80%;" />
...
### FIM: Fault Injection and Mutation for Simulink <sup>[5]</sup>

<font style="color:rgb(51, 51, 51);">论文链接：</font>[FIM: Fault Injection and Mutation for Simulink  ](https://dl.acm.org/doi/abs/10.1145/3540250.3558932)
...
<img src="https://cdn.nlark.com/yuque/0/2024/png/51796516/1734950329616-a2beda57-1e0f-4611-a26c-89ec60134900.png" style="zoom: 67%;" />

文中的图1展示了FIM的架构，包括三个主要组成部分：（1）故障库，（2）故障注入模块，（3）故障配置。  

#### 故障库

<img src="https://cdn.nlark.com/yuque/0/2024/png/51796516/1734950928935-4472dd14-b52f-4aa5-945c-dc3804602699.png" style="zoom:80%;" />
...
### ATheNA-S: A Testing Tool for Simulink Models Driven by Software Requirements and Domain Expertise<sup>[6]</sup>

论文链接： [ATheNA-S: A Testing Tool for Simulink Models Driven by Software Requirements and Domain Expertise ](https://dl.acm.org/doi/10.1145/3663529.3663804)
...
1. 自动生成适应度函数：基于软件需求规范，自动生成适应度函数，确保测试目标的全面覆盖。
   自动化适应度函数减少了手动干预，降低了设计复杂性。

2. 手动定义适应度函数：工程师可以基于自身的领域知识和具体测试需求，手动设计适应度函数。
   手动适应度函数针对特定测试目标进行优化，提高了测试的灵活性和效果。

3. 组合适应度函数：将自动生成和手动定义的适应度函数进行组合，形成一个综合的适应度函数（称为 ATheNA 适应度）。
   ATheNA 适应度能够利用自动适应度函数的全面性，同时融入手动适应度函数的针对性，提升搜索效率和生成测试用例的质量。

4. 框架化实现：ATheNA框架提供了一个易于使用的平台，工程师可以通过简单的步骤（定义类、配置函数和运行算法）完成测试用例生成。

#### 本文方法

<img src="https://cdn.nlark.com/yuque/0/2024/png/51796516/1734943359668-ae3d61d9-fa67-4de8-b600-ff54e46197c1.png" style="zoom: 67%;" />**ATheNA-S 的核心组件与实现流程**
...
<img src="https://cdn.nlark.com/yuque/0/2024/png/51796516/1734945953446-279c0e69-79e5-419b-b6ae-3baf2116290e.png" style="zoom:67%;" />
...
<img src="https://cdn.nlark.com/yuque/0/2024/png/51796516/1734944844654-a43c4580-948a-48c2-b0e9-73ef67c6a726.png" style="zoom: 67%;" />

AFC系统的假设及其要求Listing 2通过三个参数详细定义了AFC的假设：(i)每个信号的范围（第1行）、（ii）插值节点数（第2行）、以及（iii）插值函数（第3行）。此外，工程师并没有测试不同的初始条件（第4行），50𝑠作为模拟时间（第5行）。

<img src="https://cdn.nlark.com/yuque/0/2024/png/51796516/1734944889644-7ab2bfe8-c545-4f97-acf3-702160ddc1cc.png" style="zoom:67%;" />
...
**步骤1：** 工程师需要扩展类F_Assessment，定义一个合适的子类，以实现自动和手动适应度函数。在AFC示例中，工程师定义了子类AFC29_F_Assessment。自动适应度函数由方法autFitness指定，使用了Taliro（通过CallTaliro调用），该方法支持使用MTL/STL规范定义的要求。工程师提供了由Simulink模型运行生成的输出信号（o）和属性（phi）作为S-Taliro的输入，还有一些额外的配置参数被省略了。手动适应度函数manFitness旨在引导搜索向更可能导致软件故障的输入域的区域。为了优先考虑低油门值的输入信号，工程师定义了变量fit，该变量包含在模拟开始后10秒时输入信号矩阵第二列（2）中存储的值的最小值（min(i (t>=10,2))），即10秒至50秒间油门输入信号的最小值。这个值在[0, 1]范围内进行缩放，考虑油门的值范围（A(2).R）。由于搜索的目标是最小化适应度值，手动适应度函数确保在搜索过程中优先考虑低油门值的输入信号（区域）  。
  ...
  <img src="https://cdn.nlark.com/yuque/0/2024/png/51796516/1734945907944-bdf55e1b-545e-4300-b7ec-313797b348ae.png" style="zoom:67%;" />
...
<img src="https://cdn.nlark.com/yuque/0/2024/png/51796516/1734945996988-76f2eb83-a337-4e5b-848b-349141cd4860.png" style="zoom:67%;" />
...
### 相关工作三：Simulink数据集的构建与智能检索

构建Simulink数据集是开发智能检索引擎的关键，因为高质量的数据集不仅决定了引擎的训练效果，还为后续检索机制提供了标准和参考。通过涵盖多样化的模型类型和结构，数据集能够帮助智能引擎理解Simulink模型的逻辑关系、功能模式和设计规范，从而实现快速、准确的模型匹配和推荐，显著提升检索效率和用户体验。

同时，为了实现智能化的模型检索，本月调研了Simulink语料库的构建与应用，包括对EvoSL项目的大型开源Simulink模型和项目的变化管理进行研究，探讨语料库构建的可复制性以及与专有模型比较的通用性。此外，还分析了mindsdb项目在集成LLM与Mongo数据库以提高Simulink模型检索效果方面的应用实践，为模型检索与管理提供了技术支撑。

### 相关论文一

### EvoSL: A Large Open-Source Corpus of Changes in Simulink Models & Projects<sup>[7]</sup>

论文链接：[EvoSL: A Large Open-Source Corpus of Changes in Simulink Models & Projects](https://ieeexplore.ieee.org/abstract/document/10343917)
...
- 研究问题结果：图 9 展示了 EvoSL36 项目中默认分支上 Simulink 块更改的情况，按块类型分类显示了更改次数超过 50 次的块类型及其频率。
  <img src="https://github.com/Anorexia16/PicStream/releases/download/asd/20241226-115308.jpg" alt="image-20241225195426624" style="zoom:67%;" />
...
### Replicability Study: Corpora For Understanding Simulink Models & Projects<sup>[8]</sup>

论文链接：[Replicability Study: Corpora For Understanding Simulink Models & Projects](https://ieeexplore.ieee.org/abstract/document/10304867)
...
1. 语料库概述
   ...
   <img src="https://github.com/Anorexia16/PicStream/releases/download/asd/20241226-115309.jpg" alt="image-20241225200201531" style="zoom:67%;" />
...
项目链接：**[mindsdb](https://github.com/mindsdb/mindsdb)**
...
- 查询时可将预测结果与数据库数据结合返回。
  ...
  <img src="https://github.com/Anorexia16/PicStream/releases/download/asd/20241226-115311.jpg" alt="image-20241225190357338" style="zoom:67%;" />
...
思维链是人类和人工智能系统进行复杂推理的重要工具。在问题解决过程中，清晰的思维链可以帮助分解复杂问题，找到逻辑上的突破点，避免遗漏关键细节。对于人工智能来说，模拟和构建清晰的思维链可以提升系统在推理、解释和复杂任务处理中的表现，增强系统的可解释性和可靠性。思维链在大型语言模型中起到了关键作用，并且在提升模型性能和解决复杂任务中也非至关重要。

<img src="https://pica.zhimg.com/70/v2-f0236a153e21ee5bebb8755b4e70f039_1440w.image?source=172ae18b&biz_tag=Post" alt="思维链(Chain-of-Thought, CoT)的开山之作" style="zoom:80%;" />

图源：思维链(Chain-of-Thought, CoT)的开山之作

同时，为了提高在LLM+Slicing任务中使用思维链方法识别Simulink模型的准确率，本月调研了思维链方法的实际应用，包括Zero-shot-CoT方法在零样本推理任务中的表现，以及通过程序生成提示提升语言模型在复杂数学问题上的推理性能。这些研究为思维链在Simulink模型语义理解和推理任务中的扩展应用提供了理论基础。

### 相关论文一

### Large Language Models are Zero-Shot Reasoners<sup>[10]</sup>

论文链接：[Large Language Models are Zero-Shot Reasoners](https://proceedings.neurips.cc/paper_files/paper/2022/hash/8bb0d291acd4acf06ef112099c16f326-Abstract-Conference.html)
...
###  Program of Thoughts Prompting: Disentangling Computation from Reasoning for Numerical Reasoning Tasks<sup>[11]</sup>

论文链接：[ Program of Thoughts Prompting: Disentangling Computation from Reasoning for Numerical Reasoning Tasks](https://arxiv.org/abs/2211.12588)
...
通过对Simulink模型切片复用技术的调研与实现完成了以下任务：

- 实现了Simulink模型切片复现，优化了原文中的切片方法和流程。

通过对Simulink模型的测试与验证自动化的调研与实现完成了以下任务：

- 探索了基于测试序列和故障注入的仿真测试工具，研究了变异引擎在测试用例生成中的应用。
- 引入了基于适应度函数的测试优化方法，通过结合自动化和手动调整提升测试效率与覆盖率。

通过对Simulink结合大语言模型的调研与实现完成了以下任务：

- 实现了支持Simulink模型元数据的解析、存储和语义检索。
- 实现搜索引擎与MongoDB的结合环境，实现了对Simulink元数据的智能化自然语言分析。
   未来将进一步优化语义检索能力，并探索更多Simulink与大语言模型结合的可能性。

通过思维链（CoT）的调研与实现完成了以下任务：

- 实现了针对Simulink模型CoT思维链的与标注方法的设计与使用，分析了该工具的准确度等能力。

## 参考文献

1. Reicherdt, Robert, and Sabine Glesner. "Slicing MATLAB simulink models." *2012 34th International Conference on Software Engineering (ICSE)*. IEEE, 2012.
2. Cibrián, Eduardo, et al. "Towards the reuse of physical models within the development life-cycle: a case study of Simulink models." *NOMS 2022-2022 IEEE/IFIP Network Operations and Management Symposium*. IEEE, 2022.
3. Hussain, Mohammad, et al. "Approaches for simulation model reuse in systems design—A review." *SAE International Journal of Advances and Current Practices in Mobility* 4.2022-01-0355 (2022): 1457-1471.
4. Formica, Federico, et al. "Simulation-based Testing of Simulink Models with Test Sequence and Test Assessment Blocks." *IEEE Transactions on Software Engineering* (2023).
5. Bartocci, Ezio, et al. "FIM: fault injection and mutation for Simulink." *Proceedings of the 30th ACM Joint European Software Engineering Conference and Symposium on the Foundations of Software Engineering*. 2022.
6. Formica, Federico, et al. "ATheNA-S: A Testing Tool for Simulink Models Driven by Software Requirements and Domain Expertise." *Companion Proceedings of the 32nd ACM International Conference on the Foundations of Software Engineering*. 2024.
7. Shrestha, Sohil Lal, et al. "EvoSL: a large open-source corpus of changes in Simulink models & projects." *2023 ACM/IEEE 26th International Conference on Model Driven Engineering Languages and Systems (MODELS)*. IEEE, 2023.
8. Shrestha S L, Chowdhury S A, Csallner C. Replicability study: Corpora for understanding simulink models & projects[C]//2023 ACM/IEEE International Symposium on Empirical Software Engineering and Measurement (ESEM). IEEE, 2023: 1-12.
9. MindsDB. (n.d.). *MindsDB: The easiest way to add AI to your apps*. Retrieved from https://github.com/mindsdb/mindsdb
10. Kojima T, Gu S S, Reid M, et al. Large language models are zero-shot reasoners[J]. Advances in neural information processing systems, 2022, 35: 22199-22213.
